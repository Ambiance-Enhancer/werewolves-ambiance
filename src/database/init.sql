...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'role_enum') THEN
      CREATE TYPE werewolves.role_enum AS ENUM (
        'villageois',
        'loup_garou',
        'petite_fille',
        'chasseur',
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'game_status_enum') THEN
      CREATE TYPE werewolves.game_status_enum AS ENUM ('initializing', 'waiting', 'in_progress', 'completed');
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'period_enum') THEN
        ...
        CREATE TYPE werewolves.period_enum AS ENUM (
        'start_up',
        'cupidon',
        'amoureux',
        'voleur',
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'action_type_enum') THEN
        CREATE TYPE werewolves.action_type_enum AS ENUM (
        'vote',
        'kill',
        'heal',
        'poison',
...
CREATE TABLE werewolves.players (
    ...
    CONSTRAINT ck_players_no_self_lover CHECK (
        lover_player_id IS null OR lover_player_id != id
    )
);
...
CREATE INDEX idx_players_game_alive ON werewolves.players (
    game_id, is_alive
) WHERE is_alive;
...
CREATE TABLE werewolves.logs (
    ...
    target_player_id uuid NOT NULL REFERENCES werewolves.players (
        id
    ) ON DELETE CASCADE,
    created_at timestamptz NOT NULL DEFAULT now()
);
...
CREATE INDEX idx_logs_game_round_period ON werewolves.logs (
    game_id, round_number, period
);
