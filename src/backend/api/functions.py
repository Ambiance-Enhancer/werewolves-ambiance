...
def process_day(game: Game) -> None:
    ...
    if chasseur and not chasseur.alive and chasseur.revenge_target is None:
        ...
        if target:
            ...
            click.echo(f"💥 {chasseur.name} shoots {target.name} with their dying breath!")
        else:
    ...
    if current_mayor and not current_mayor.alive:
        ...
        if successor:
            ...
            click.echo(f"👑 {current_mayor.name} has appointed {successor.name} as the new Mayor.")
            current_mayor = successor
        else:
            click.echo("No successor selected. The Village remains without a Mayor for now (or a new election will occur).")
            current_mayor.is_mayor = False
            current_mayor = None
    ...
    if eliminated and eliminated.is_mayor:
        ...
        if alive_players:
            ...
            if successor:
                ...
                successor.is_mayor = True
                click.echo(f"👑 {eliminated.name} has appointed {successor.name} as the new Mayor.")
            else:
                click.echo("No successor selected. The Village remains without a Mayor.")
                eliminated.is_mayor = False
